...
public class JsfLoginUrlAuthenticationEntryPoint implements AuthenticationEntryPoint, InitializingBean {
    ...
    /**
     * Set to true to force login form access to be via https. If this value is true (the default is
     * false), and the incoming request for the protected resource which triggered the interceptor
     * was not already <code>https</code>, then the client will first be redirected to an https URL,
     * even if <tt>serverSideRedirect</tt>
    ...
    /**
     * The URL where the <code>UsernamePasswordAuthenticationFilter</code> login page can be found.
     * Should either be relative to the web-app context path (include a leading {@code /}) or an
     * absolute URL.
     */
     ...
     *
     * @param useForward true if a forward to the login page should be used. Must be false (the
     * default) if {@code loginFormUrl} is set to an absolute value.
     */
